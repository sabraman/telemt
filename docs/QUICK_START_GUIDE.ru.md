...
> Так же замените значение параметра tls_domain на реальный HTTPS-сайт, который правдоподобен для региона и хостинг-провайдера вашего сервера.
> Если вы задаёте `mask_host`, держите его привязанным к тому же донору, если только вы намеренно не уводите masking-трафик на другой адрес.
